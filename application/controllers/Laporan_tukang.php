...
class Laporan_tukang extends CI_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model('Laporan_tukang_model');
        $this->load->library('form_validation');
        $this->session->set_flashdata('title', 'Laporan | CV REJEKI SUMBER TEKNIK');
    ...
    public function index()
    {
        ...
        $this->template->load('template', 'laporan_tukang/laporan');
    }

    public function ajax_list()
    {
        $list = $this->Laporan_tukang_model->get_datatables();
        $data = array();
        $no   = $_POST['start'];
        $sum  = 0;
        foreach ($list as $customers) {
            ...
            $row[]  = '<a class="btn btn-sm btn-primary" href="' . base_url('laporan_tukang/print/' . $customers->nospk) . '" title="Edit" target="_blank">Print</a>';
            $data[] = $row;
        ...
        $output = array(
            "draw"            => $_POST['draw'],
            "recordsTotal"    => $this->Laporan_tukang_model->count_all(),
            "recordsFiltered" => $this->Laporan_tukang_model->count_filtered(),
            "data"            => $data,
            // "sum"             => $sum,
        );
    ...
    $this->template->load('template', 'laporan_tukang/laporan_read', $data);
    ...
    public function pdf()
    {
        ...
        $nourut = 1;
        foreach ($this->Laporan_tukang_model->excel($tanggal_a, $tanggal_b) as $data) {
    ...
    public function report(){
        ...
        $this->pdfgenerator->load_view('laporan/laporan_pdf', $data);
    }

    public function print($nospk){
        
        $data = array('nospk' => $nospk);

        $this->pdfgenerator->setPaper('A4', 'landscape');
        $this->pdfgenerator->filename = "laporan.pdf";
        $this->pdfgenerator->load_view('laporan/laporan_print', $data);
    }
